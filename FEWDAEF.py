...
# Constants
EMAILS_FILE = "FEWDAEF.json"       # JSON file to store collected emails with timestamps
    ...
    while len(result_emails) < TARGET_COUNT:
        for term in search_terms:
            ...
            for app_summary in apps:
                ...
                installs_text = app_summary.get('installs', '')

                # Skip popular apps based on summary data
                if 'M' in installs_text or 'B' in installs_text:
                    print(f"[DEBUG] Skipping popular app: {app_summary.get('title')} with installs: {installs_text}")
                    continue
                ...
                # Filter emails to include only those ending with @gmail.com
                if not email.lower().endswith("@gmail.com"):
                    print(f"[DEBUG] Skipping non-Gmail email: {email}")
                    continue
